...
struct neigh_table nd_tbl = {
	...
	.fill_info =	addrconf_fill_info,
	.nlmsg_size =	addrconf_nlmsg_size,
...
static void ndisc_destructor(struct neighbour *neigh)
{
	...
	timer_shutdown_sync(&neigh->pio_timer);

	list_for_each_entry_safe(pio, tmp, &neigh->pio_list, list) {
		atomic_dec(&idev->pio_count);
		/* list_del_rcu is pointless in the destructor, but let's
		 * make sure the refcount on the neigh goes to zero
		 */
