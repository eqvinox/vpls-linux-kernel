...
#define IPV6_MAX_PIO_PER_NEIGH		16
				      ...
				      unsigned long expires)
{
	...
	if (!pio) {
		if (total >= IPV6_MAX_PIO_PER_NEIGH) {
			pr_info_ratelimited("%s: neighbor %pI6c reached prefix count limit (%d), dropping %pI6c",
					    neigh->dev->name, naddr, IPV6_MAX_PIO_PER_NEIGH,
					    &pinfo->prefix);
			return;
		}
		...
		pio = kcalloc(1, sizeof(*pio), GFP_ATOMIC);
...
void addrconf_pio_timer(struct timer_list *t)
{
	...
	bool leftover = false;
	unsigned long abs_leftover, pio_expires;
	struct in6_addr *naddr = (struct in6_addr *)&neigh->primary_key;
	...
	list_for_each_entry_safe(pio, next, &neigh->pio_list, list) {
		...
		if (time_after(pio_expires, jiffies)) {
			if (!leftover || time_before(pio_expires, abs_leftover)) {
				leftover = true;
				abs_leftover = pio_expires;
			}
			continue;
		}

		pr_debug("%s: %pI6c: del prefix %pI6c\n",
			 neigh->dev->name, naddr, &pio->prefix);
		list_del_rcu(&pio->list);
		kfree_rcu(pio, rcu);
	}

	if (leftover)
		mod_timer(&neigh->pio_timer, abs_leftover);
